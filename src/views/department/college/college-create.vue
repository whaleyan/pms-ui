<template>
    <span>
        <Button type="primary" icon="plus" @click="openDialog">添加学院</Button>
        <!-- 添加学院 -->
        <Modal v-model="show" title="添加学院" :loading="loading" @on-ok="onOk" @on-visible-change="visibleChange">
            <p>
                <Form ref="createForm" :model="item" :rules="rules" :label-width="80">
                    <FormItem label="学科名称" prop='depName'>
    ...
    methods: {
        onOk() {
            ...
            this.$refs.createForm.validate((valid) => {
                if (valid) {
                    departmentService.addDepartment(params).then(res => {
                        if (res.status === 200) {
